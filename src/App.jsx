...
function App() {
  ...
  return (
    <div className="w-screen h-screen bg-gray-900 flex items-center justify-center overflow-hidden">
      {/* Container to maintain 16:9 aspect ratio and scale to fit */}
      <div 
        className="relative bg-canvas shadow-2xl flex-shrink-0"
        style={{
          width: '1920px',
          height: '1080px',
      ...
      <div className="fixed bottom-4 text-white/50 text-sm">
        Slide {currentSlide + 1} of {slides.length} &middot; Use arrow keys to navigate
      </div>
    </div>
  );
}
...
export default function ScaledApp() {
  ...
  return (
    <div className="w-screen h-screen bg-gray-900 flex items-center justify-center overflow-hidden">
      ...
      >
        <AppInner currentSlide={0} /> {/* We need to refactor slightly so state belongs to parent or inner */}
      </div>
    </div>
  );
}

// Refactored to handle state cleanly
function AppInner() {
  const [currentSlide, setCurrentSlide] = useState(0);
